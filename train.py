...
raw_dataset = load_dataset("json", data_files={"train": TRAIN_DATA_PATH}, split="train")
logger.info(f"Loaded {len(raw_dataset)} training samples.")

# Filter out empty or very short outputs
raw_dataset = raw_dataset.filter(
    lambda ex: ex.get("output", None) and len(ex["output"].strip()) > 10
)
logger.info(
...
def formatting_prompts_func(example):
    ...
    Args:
        example (dict): A dictionary with 'instruction', 'input', and 'output' keys.
...
eval_dataset = eval_dataset.map(formatting_prompts_func, desc="Formatting eval prompts")
...
empty_outputs = sum(1 for ex in raw_dataset if not ex.get("output", "").strip())
logger.info(f"Number of empty outputs: {empty_outputs}")
...
training_args = TrainingArguments(
    ...
    # Ensure warmup_steps is an integer
